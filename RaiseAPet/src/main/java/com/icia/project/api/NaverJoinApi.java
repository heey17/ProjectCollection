...
public class NaverJoinApi {
	...
	private final static String CLIENT_ID = "R1xjGhB4w40wTddIWpXU";
	private final static String CLIENT_SECRET = "snWgS93F8w";

	// 여기 주소 값이 NaverController의 callback 호출 처리 메소드 주소값 부분과 일치해야 함.
	// (네이버 개발자 페이지의 주소도 동일해야 함.)

	// URL에 설정한 정보입니다.
	// redirect_uri: 네이버 로그인 인증의 결과를 전달받을 콜백 URL(URL 인코딩). 
	// 애플리케이션을 등록할 때 Callback
	private final static String REDIRECT_URI = "http://106.243.194.230:9092/project/naverjoinok";
	...
	public String getUserProfile(OAuth2AccessToken oauthToken) throws IOException {
		...
		OAuth20Service oauthService = new ServiceBuilder().apiKey(CLIENT_ID)
														  ...
														  .build(NaverApi.instance());// NaverLoginApi 클래스를 싱글톤으로 호출 
