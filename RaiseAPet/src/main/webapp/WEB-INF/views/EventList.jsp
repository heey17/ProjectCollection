...
<table style="text-align:center; margin:2% auto; width:60%;">
	<c:choose>
		<c:when test="${empty sessionScope.loginUser.userId}">
		    <tr><td><button onclick="location.href='memberLoginForm'"><img src="resources/eventFile/miniGame.png"/></button></td></tr>	
		</c:when>
		<c:otherwise>
			<tr><td><button onclick="window.open('eventGame', 'mywindow', 'width=800, height=600');"><img src="resources/eventFile/miniGame.png"/></button></td></tr>		
		</c:otherwise>
	</c:choose>
	
	<c:forEach var="event" items="${eventList}">
		<tr style="float:left; margin:3%"><td>
		<button onclick="location.href='eventView?eventNum=${event.eventNum}&eventHit=${event.eventHit}'">
		<img src="resources/eventFile/${event.eventTitleImage}" style="  width:300px; height:250px; ">
		</button>
		</td></tr>
	</c:forEach>
</table>
