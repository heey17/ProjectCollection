...
<div style="width:50%; height:80%; margin:4% auto;">
   		...
   		<button onclick="window.open('designerInformation?designerCode=${beauty.designerCode}','window팝업','width=500, height=600, menubar=no, status=no, toolbar=no');">
   		<img src="resources/beautyFile/${beauty.designerImage}" style=" border-radius:30%; width:300px; height:300px"/></button><br/><br/>
   		디자이너&nbsp;${beauty.designerName}<br/>
   		
   		<c:choose>
   		
   		<c:when test="${not empty sessionScope.loginUser.userId}">
   		<button style="color:#DDC3F7" onclick="window.open('beautyReservationForm?designerName=${beauty.designerName}&designerOff=${beauty.designerOff}&designerCode=${beauty.designerCode}','window팝업','width=500, height=500, menubar=no, status=no, toolbar=no');">예약하기</button>
   		</c:when>
